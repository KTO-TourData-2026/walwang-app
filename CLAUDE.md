...
- **커밋·이슈·PR을 직접 올리지 않는다.** `git commit`, GitHub 이슈 생성, pull request 생성을 스스로 실행하지 않는다.
- 사용자가 "커밋/이슈/PR 작성하라"고 시키면 **초안만 작성해서 사용자에게 컨펌을 받는다.** 컨펌 전에는 절대 실제로 올리지(실행하지) 않는다.
- 어떤 경우에도 사용자 확인 없이 이 3가지를 실행하지 않는다.
